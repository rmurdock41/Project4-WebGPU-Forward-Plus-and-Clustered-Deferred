...
class CameraUniforms {
    readonly buffer = new ArrayBuffer(60 * 4);  
    ...
    set screenSize(size: { width: number; height: number }) {
        this.floatView[32] = size.width;
        this.floatView[33] = size.height;
    }
    ...
    set viewMat(mat: Float32Array) {
        this.floatView.set(mat.subarray(0, 16), 36);
    }
...
export class Camera {
    ...
    constructor () {
        ...
        this.uniformsBuffer = device.createBuffer({
            size: this.uniforms.buffer.byteLength, // 64 bytes
    ...
    onFrame(deltaTime: number) {
        ...
        this.uniforms.screenSize = { width: canvas.width, height: canvas.height };
